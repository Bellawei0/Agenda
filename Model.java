 ...
 */
public class Model {
    TreeMap<LocalDate, TreeSet<Event>> data;
    ...
    public TreeSet<Event> getEvents(LocalDate date){
   ...
   // add the Events to the data
    public void update(LocalDate date,Event event) {
        if(!data.containsKey(date)){
            TreeSet<Event> events = new TreeSet<Event>();
            data.put(date, events);
        }
        data.get(date).add(event);
    ...
    void loadFile(Scanner scanner) {
        ...
        while (scanner.hasNextLine()) {
            ...
            LocalDate startD = LocalDate.of(year, startingMonth, 1);
            LocalDate endD = LocalDate.of(year, endingMonth, 31);
