...
//        }
    ...
    public void stateChanged(ChangeEvent e) {
        ...
        //Clone from treeMap
        TreeMap<LocalDateTime, TreeSet<Event>> eventMap = model.getData();

        // Get Keys from treeMap
        ArrayList<LocalDateTime> keyEventList = new ArrayList<>();
        for (Entry<LocalDateTime, TreeSet<Event>> entry : eventMap.entrySet()) {
        ...
        for (int j = 0; j < currentEventkey.size(); j++) {
            Event event = eventMap.get(currentEventkey.get(j)).first();
